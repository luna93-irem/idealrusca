/* ============================================================
   Rusça Öğren — v2
   Yenilikler:
     - MyMemory API ile otomatik çeviri (sözlükte olmayan kelimeler)
     - Çeviri önbelleği (localStorage)
     - Tıkla-sabitle popover davranışı
     - Popover'da "+Ekle" butonu aktif çalışır
   ============================================================ */
...
/* ============================================================
   2) ÇEVİRİ (MyMemory API + önbellek)
   ============================================================ */
const TRANS_KEY = 'rusca-trans-v1';
let TRANS_CACHE = {};
...
async function translateRuToTr(word) {
  const key = norm(word);
  if (TRANS_CACHE[key]) return TRANS_CACHE[key];

  try {
    const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(word)}&langpair=ru|tr`;
    const res = await fetch(url);
    if (!res.ok) throw new Error('API ' + res.status);
    const data = await res.json();
    let tr = data?.responseData?.translatedText || '';
    tr = tr.trim();
    // MyMemory bazen hatayı string döner
    if (!tr || /^MYMEMORY WARNING/i.test(tr) || norm(tr) === norm(word)) {
      return null;
    }
    // Baş harf düzenleme
...
function renderHome() {
  ...
  $('#page-home').innerHTML = `
    <div class="hero">
      ...
      <p>Kural tabanlı morfoloji, EPUB/PDF okuyucu, otomatik çeviri, kişisel kütüphane ve flash kartlar — tek yerde.</p>
    ...
    <div class="features">
      ...
      <button class="feature" data-go="reader"><div class="icon">📖</div><h3>Okuyucu</h3><p>EPUB/PDF yükle, kelimeye tıkla → analiz + çeviri</p></button>
      <button class="feature" data-go="library"><div class="icon">📚</div><h3>Kütüphanem</h3><p>${LIBRARY.length} kayıtlı kelime</p></button>
    ...
    <div class="card" style="margin-top:20px">
      ...
      <ul style="margin:8px 0 0 20px; padding:0; font-size:14px; line-height:1.9">
        <li><b>Okuyucu</b>'dan EPUB/PDF yükle</li>
        <li>Bir kelimeye <b>tıkla</b> → pencere sabitlenir · sözlük + çeviri + çekim tablosu</li>
        <li>Otomatik çeviri <b>MyMemory</b>'den gelir (internet gerekir)</li>
        <li>Beğendiğin kelimeyi <b>"+ Ekle"</b> ile kütüphanene kaydet</li>
        <li><b>Kütüphanem</b>'den flash kartlarla çalış</li>
      </ul>
    </div>
  `;
...
function renderReader() {
  $('#page-reader').innerHTML = `
    <div class="page-head">
      ...
      <p>Bir kitap yükle. Kelimeye <b>tıkla</b> → pencere sabitlenir: kök, cinsiyet, hâl, çekim tablosu ve <b>otomatik çeviri</b>. Beğendiklerini kütüphanene ekle.</p>
    </div>
    <div class="dropzone" id="dz">
      <div class="dz-icon">📚</div>
      <h3>EPUB veya PDF dosyanı bırak</h3>
      <p>ya da tıkla · tarayıcında işlenir, sunucuya gönderilmez</p>
  ...
  $('#readerArea').innerHTML = `
    <div class="book-info">
      <div class="bk">
        ...
        <div class="bk-meta">${text.length.toLocaleString('tr-TR')} karakter · ~${wordCount.toLocaleString('tr-TR')} Rusça kelime · Kelimeye <b>tıkla</b> → pencere sabitlenir</div>
      </div>
      <button class="btn ghost small" id="closeBook">Kapat</button>
    </div>
    <div class="reader-text" id="readerText">${html}</div>
  `;
  $('#closeBook').onclick = () => { currentBook = null; $('#readerArea').innerHTML = ''; $('#dz').style.display = ''; hidePopover(); };
...
/* ============================================================
   17) POPOVER (Tıkla-sabitle)
   ============================================================ */
const popover = $('#popover');
let popState = { word: null, pinned: false, targetEl: null, analysis: null, loading: false };
...
function bindReaderEvents() {
  ...
  rt.addEventListener('mouseover', e => {
    ...
    if (!w) return;
    if (popState.pinned) return; // sabitlenmişken hover değiştirmez
    showPopover(w, false);
  });
  ...
  rt.addEventListener('mouseout', e => {
    ...
    // Popover üzerine geçmediyse kapat
    setTimeout(() => {
      if (!popState.pinned && !popover.matches(':hover')) hidePopover();
    }, 120);
  });
  ...
  rt.addEventListener('scroll', () => { if (!popState.pinned) hidePopover(); });
}

async function showPopover(wEl, pinned) {
  const raw = wEl.dataset.w;
  const analysis = analyzeWord(raw);
  if (!analysis) return;

  // Aynı kelime ve zaten sabitliyse → hiçbir şey yapma
  if (popState.word === analysis.word && popState.pinned && pinned) return;

  // Önceki active class temizle
  ...
  // Eğer çeviri önbellekte varsa uygula
  const cached = TRANS_CACHE[norm(analysis.word)];
  if (!analysis.tr && cached) analysis.tr = cached;

  renderPopover();
  popover.classList.add('show');
  popover.classList.toggle('pinned', popState.pinned);
  positionPopover(wEl);

  // Çeviri yoksa ve önbellekte de yoksa → çek
  if (!analysis.tr) {
    fetchTranslationAsync(analysis.word);
  }
}
...
async function fetchTranslationAsync(word) {
  ...
  const tr = await translateRuToTr(word);
  popState.loading = false;

  // Popover kapanmış veya kelime değişmişse güncelleme
  if (popState.word !== norm(word) && popState.word !== word) {
    // belki farklı harf durumu, yine de devam edelim ama popover açık mı kontrol
  }

  if (tr) {
    popState.analysis.tr = tr;
    // DICT'de geçici olarak güncelle ki tekrar açtığımızda hazır olsun
...
function renderPopover() {
  ...
  popover.innerHTML = `
    ...
    ${!popState.pinned ? '<div class="p-hint">Sabitlemek için tıkla</div>' : '<div class="p-hint">Dışına tıkla → kapanır</div>'}
  `;

  // Olaylar (event bubbling'i durdur → click-outside kapatmasın)
  popover.querySelector('#pClose')?.addEventListener('click', e => {
    e.stopPropagation();
    hidePopover();
  });
...
// Global click: popover veya kelime dışında bir yere tıklanırsa kapat
...
function renderLibrary() {
  ...
  page.innerHTML = `
    <div class="page-head"><h1>Kütüphanem</h1><p>Eklediğin kelimeler. Arama, silme, dinleme, flash kart pratiği.</p></div>
    ...
    <div id="libBody">${LIBRARY.length ? renderLibGrid(LIBRARY) : '<div class="lib-empty">Henüz kelime eklemedin.<br>Okuyucudan bir kitap yükleyip kelimelere tıklayarak ekleyebilirsin.</div>'}</div>
  `;
  $('#libSearch').oninput = e => {
    const q = norm(e.target.value);
    const filtered = q ? LIBRARY.filter(x => norm(x.lemma).includes(q) || norm(x.tr).includes(q)) : LIBRARY;
    $('#libBody').innerHTML = filtered.length ? renderLibGrid(filtered) : '<div class="lib-empty">Eşleşme yok.</div>';
    bindLibActions();
  };
