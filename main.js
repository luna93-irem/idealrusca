/* ============================================================
   Rusça Öğren — v3
   Değişiklikler:
     - OpenRussian API yerine MyMemory (çeviri) + Google TTS (ses)
     - Popover davranışı: kelime üzerinde açık kalır
       · başka kelimeye geçince kapanır
       · boşluğa tıklayınca kapanır
       · ✕ butonu ile kapanır
     - Dinleme artık Google Translate TTS'den MP3 olarak gelir
   ============================================================ */
...
/* ============================================================
   2) ÇEVİRİ (MyMemory + localStorage önbellek)
   ============================================================ */
const TRANS_KEY = 'rusca-trans-v2';
let TRANS_CACHE = {};
...
async function translateRuToTr(word) {
  ...
  try {
    ...
    tr = tr.trim();
    if (!tr || /^MYMEMORY WARNING/i.test(tr) || norm(tr) === norm(word)) return null;
...
/* ============================================================
   3) SES — Google Translate TTS (MP3 URL)
   ============================================================ */
...
function googleTtsUrl(text) {
  return 'https://translate.google.com/translate_tts'
    ...
    + '&tl=ru'
    + '&client=tw-ob'; // web client → CORS'suz çalışır
}
...
function speak(text) {
  ...
  try {
    // Aynı sesi tekrar tekrar oluşturmayalım
    ...
    // Sınırlı önbellek
    if (AUDIO_CACHE.size > 60) {
      const firstKey = AUDIO_CACHE.keys().next().value;
      AUDIO_CACHE.delete(firstKey);
    }
    audio.currentTime = 0;
    audio.play().catch(() => {
      // Bazı tarayıcılar doğrudan oynatmayı engeller → son çare Web Speech
      fallbackSpeak(text);
    });
  } catch (e) {
    fallbackSpeak(text);
...
function renderHome() {
  ...
  $('#page-home').innerHTML = `
    ...
    <div class="features">
      ...
      <button class="feature" data-go="reader"><div class="icon">📖</div><h3>Okuyucu</h3><p>EPUB/PDF yükle, kelimeye tıkla → analiz + çeviri + ses</p></button>
    ...
    <div class="card" style="margin-top:20px">
      ...
      <ul style="margin:8px 0 0 20px; padding:0; font-size:14px; line-height:1.9">
        ...
        <li>Bir kelimeye <b>tıkla</b> → pencere açık kalır · sözlük + çeviri + çekim tablosu</li>
        <li>Başka bir kelimeye geç, boşluğa tıkla veya ✕'e bas → kapanır</li>
        <li>Otomatik çeviri <b>MyMemory</b>'den gelir</li>
        <li>Ses <b>Google Translate TTS</b>'den MP3 olarak çalınır</li>
...
function renderReader() {
  $('#page-reader').innerHTML = `
    <div class="page-head">
      ...
      <p>Bir kitap yükle. Kelimeye <b>tıkla</b> → pencere açık kalır: kök, cinsiyet, hâl, çekim tablosu, <b>otomatik çeviri</b> ve <b>ses</b>. Başka kelimeye geç veya ✕'e bas → kapanır.</p>
    </div>
  ...
  $('#readerArea').innerHTML = `
    <div class="book-info">
      <div class="bk">
        ...
        <div class="bk-meta">${text.length.toLocaleString('tr-TR')} karakter · ~${wordCount.toLocaleString('tr-TR')} Rusça kelime · Kelimeye <b>tıkla</b> → pencere açık kalır</div>
      </div>
...
/* ============================================================
   17) POPOVER — Yeni davranış
     · Kelime üzerindeyken açık kalır (mouseout kapatmaz)
     · Başka kelimeye geçince kapanır
     · Boşluğa tıklayınca kapanır
     · ✕ butonuna basınca kapanır
   ============================================================ */
const popover = $('#popover');
let popState = { word: null, targetEl: null, analysis: null, loading: false, open: false };
...
function bindReaderEvents() {
  ...
  rt.addEventListener('mouseover', e => {
    ...
    if (!w) return;
    if (popState.targetEl === w) return; // aynı kelime → yeniden açma
    showPopover(w);
  });
  ...
  rt.addEventListener('scroll', () => {
    // Popover hedef kelimenin yanında kalsın diye yeniden konumlandır
...
async function showPopover(wEl) {
  ...
  // Önbellekten çeviri varsa uygula
  ...
  popover.classList.remove('pinned'); // artık pin yok; açık kalma mantığı farklı
  positionPopover(wEl);
...
async function fetchTranslationAsync(word) {
  ...
  // Popover hâlâ aynı kelime için açık mı?
...
// Belge tıklaması: popover veya kelime dışında bir yere tıklanınca kapat
