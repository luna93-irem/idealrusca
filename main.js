/* ============================================================
   Rusça Öğren — v6
   Değişiklikler:
     - Popover küçük + accordion (çekim, cümle, alternatif anlamlar kapalı başlar)
     - Fare kelimeden çekilince küçük gecikmeyle kapanır
     - Kelimeye tıklayınca kilitlenir
     - Popover kelimenin yanına konumlanır (arkadaki metni kapatmaz)
     - Diğer her şey aynı: alfabe, gramer, kütüphane, pratik, EPUB/PDF,
       IndexedDB kitap deposu, kaldığı yerden devam, çeviri zinciri,
       alternatif anlamlar, Google TTS.
   ============================================================ */
  ...
  $('#page-home').innerHTML = `
    ...
    <div class="card" style="margin-top:20px">
      ...
      <ul style="margin:8px 0 0 20px; padding:0; font-size:14px; line-height:1.9">
        ...
        <li>Kelimeye <b>tıkla</b> → küçük pencere açılır: çeviri + etiketler · <b>📐 / 📖 / 📚</b> butonlarıyla detaylar</li>
        <li>Pencere metni kapatmasın diye kelimenin <b>yanına</b> konumlanır</li>
...
function renderReader() {
  $('#page-reader').innerHTML = `
    <div class="page-head">
      ...
      <p>Kitap yükle → otomatik kaydedilir. Kaldığın yerden devam etmek için Kütüphanem'den aç. Kelimeye <b>tıkla</b> → küçük pencere açılır, kitabı kapatmaz.</p>
    </div>
...
/* ============================================================
   18) POPOVER — küçük, accordion, fare-çekilince-kapanır
   ============================================================ */
const popover = $('#popover');
let popState = { word: null, targetEl: null, analysis: null, loading: false, open: false, pinned: false, sentence: '', meanings: [] };
let _hideTimer = null;
...
// Popover üzerine gelince kapanma
...
function bindReaderEvents() {
  ...
  rt.addEventListener('click', e => {
    ...
    if (!w) return;
    e.stopPropagation();
    showPopover(w, true); // tıkla → kilitle
  });
...
function renderPopover() {
  ...
  popover.innerHTML = `
    ...
    <div class="p-actions">
      ...
      <button data-act="add" class="${inLib ? 'added' : ''}" ${inLib ? 'disabled' : ''}>${inLib ? '✓' : '+ Ekle'}</button>
    </div>

    ${popState.sentence ? `
      <button class="p-toggle" data-panel="sentence">📖 Cümleyi çevir ▸</button>
      <div class="p-panel" id="panel-sentence">
        <div class="p-sent-text">${popState.sentence}</div>
        <button class="p-sent-btn" id="sentenceBtn">Çevir</button>
        <div id="sentenceTrans"></div>
      </div>
    ` : ''}

    ${formsHTML ? `
      <button class="p-toggle" data-panel="forms">📐 Çekim tablosu ▸</button>
      <div class="p-panel" id="panel-forms">${formsHTML}</div>
    ` : ''}

    ${meaningsHTML ? `
      <button class="p-toggle" data-panel="meanings">📚 Diğer anlamlar ▸</button>
  ...
  popover.querySelectorAll('.p-toggle').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
      const panel = popover.querySelector('#panel-' + btn.dataset.panel);
      if (!panel) return;
      const isOpen = panel.classList.toggle('open');
      const base = btn.textContent.replace(/[▸▾]\s*$/, '').trim();
      btn.textContent = base + ' ' + (isOpen ? '▾' : '▸');
      positionPopover(popState.targetEl);
    });
  });
...
function positionPopover(target) {
  ...
  // Önce sağ tarafa dene
  let left = r.right + 10;
  let top  = r.top;

  // Sağa sığmıyorsa sola koy
  if (left + pw > window.innerWidth - 8) {
    left = r.left - pw - 10;
  }
  // Sola da sığmıyorsa (çok dar ekran) kelimenin altına
  if (left < 8) {
