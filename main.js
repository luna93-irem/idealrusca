...
function heuristicAnalyze(word) {
  const endings = [
    { e:'ами',  c:'ins', n:'pl' }, { e:'ями', c:'ins', n:'pl' },
    { e:'ах',   c:'pre', n:'pl' }, { e:'ях',  c:'pre', n:'pl' },
    { e:'ам',   c:'dat', n:'pl' }, { e:'ям',  c:'dat', n:'pl' },
    { e:'ов',   c:'gen', n:'pl' }, { e:'ев',  c:'gen', n:'pl' },
    { e:'ей',   c:'gen', n:'pl' },
    { e:'ого',  c:'gen', n:'sg', pos:'adj' },
    { e:'его',  c:'gen', n:'sg', pos:'adj' },
    { e:'ому',  c:'dat', n:'sg', pos:'adj' },
    { e:'ему',  c:'dat', n:'sg', pos:'adj' },
    { e:'ыми',  c:'ins', n:'pl', pos:'adj' },
    { e:'ими',  c:'ins', n:'pl', pos:'adj' },
    { e:'ый',   c:'nom', n:'sg', pos:'adj' },
    { e:'ий',   c:'nom', n:'sg', pos:'adj' },
    { e:'ой',   c:'ins', n:'sg' },
    { e:'ая',   c:'nom', n:'sg', pos:'adj' },
    { e:'яя',   c:'nom', n:'sg', pos:'adj' },
    { e:'ое',   c:'nom', n:'sg', pos:'adj' },
    { e:'ее',   c:'nom', n:'sg', pos:'adj' },
    { e:'ые',   c:'nom', n:'pl', pos:'adj' },
    { e:'ие',   c:'nom', n:'pl', pos:'adj' },
    { e:'ей',   c:'ins', n:'sg' },
    { e:'ом',   c:'ins', n:'sg' },
    { e:'ем',   c:'ins', n:'sg' },
    { e:'а',    c:'gen', n:'sg' },
    ...
    { e:'ы',    c:'nom', n:'pl' },
    { e:'и',    c:'nom', n:'pl' },
    { e:'ла',   c:'—',   n:'sg', pos:'verb', tense:'past' },
    { e:'ло',   c:'—',   n:'sg', pos:'verb', tense:'past' },
    { e:'ли',   c:'—',   n:'pl', pos:'verb', tense:'past' },
    { e:'л',    c:'—',   n:'sg', pos:'verb', tense:'past' },
    { e:'ть',   c:'—',   n:'sg', pos:'verb', tense:'inf' }
  ];
...
function addToLibrary(entry) {
  ...
  if (LIBRARY.some(x => norm(x.lemma || x.word) === key)) { toast('Bu kelime zaten kütüphanede'); return false; }
  LIBRARY.unshift({
...
function renderHome() {
  ...
  $('#page-home').innerHTML = `
    ...
    <div class="features">
      ...
      <button class="feature" data-go="practice"><div class="icon">🎯</div><h3>Pratik</h3><p>Sonsuz soru + her sorunun altında açıklama</p></button>
    </div>
    ...
    <div class="card" style="margin-top:20px">
      ...
      <ul style="margin:8px 0 0 20px; padding:0; font-size:14px; line-height:1.9">
        ...
        <li>Okurken bir kelimenin <b>üzerine gel</b> → kural tabanlı analiz açılır</li>
        <li>Beğendiğin kelimeyi <b>"+ Kütüphaneme ekle"</b> ile kaydet</li>
        <li><b>Kütüphanem</b> sekmesinden flash kartlarla çalış</li>
        <li><b>Pratik</b> sekmesinde her sorunun altında <b>"Neden doğru/yanlış"</b> açıklaması görünür</li>
      </ul>
    </div>
  `;
  ...
  'Cinsiyet': {
    body: `
      <div class="card">
        ...
        <div class="callout"><b>İpucu:</b> -ость, -сть, -нь, -жь, -чь, -шь, -щь ile bitenler <b>dişil</b>'dir.</div>
      </div>`
  },
  'Çoğul': {
    body: `
  ...
  'Fiiller': {
    body: `
      ...
      <div class="card">
        ...
        <div class="callout"><b>Kural:</b> Geçmiş zaman -л + cinsiyet eki.</div>
      </div>`
  },
  'Sıfatlar': {
    body: `
    ...
    body: `
      <div class="card">
        ...
        <table class="data">
          ...
          <tr><td class="ru">в / на (yer)</td><td>Prepositional</td><td class="ru">в Москве</td></tr>
          <tr><td class="ru">в / на (yön)</td><td>Accusative</td><td class="ru">в Москву</td></tr>
          <tr><td class="ru">у</td><td>Genitive</td><td class="ru">у меня</td></tr>
...
function renderReader() {
  $('#page-reader').innerHTML = `
    <div class="page-head">
      ...
      <p>Bir kitap yükle. Her kelimenin üzerine geldiğinde <b>kural tabanlı analiz</b> açılır: kök, cinsiyet, hâl, çekim.</p>
    </div>
    <div class="dropzone" id="dz">
      <div class="dz-icon">📚</div>
      <h3>EPUB veya PDF dosyanı buraya bırak</h3>
      <p>ya da tıkla ve seç · Dosya tarayıcında işlenir</p>
...
function showReader() {
  ...
  const html = tokens.map(tok => {
    ...
    if (!m) return tok;
    return m[1] + `<span class="w" data-w="${m[2]}">${m[2]}</span>` + m[3];
  }).join('');
...
function bindReaderHover() {
  ...
  rt.addEventListener('mouseover', e => {
    ...
    if (w) showPopoverForWord(w);
  });
  rt.addEventListener('mouseout', e => {
    if (e.target.closest('.w')) hidePopover();
  });
...
function showPopoverForWord(wEl) {
  ...
  if (analysis.allForms && analysis.pos === 'noun') {
    ...
    detail = `
      <div class="p-detail">
        <b>Şimdiki:</b> ${analysis.allForms.pr.join(', ')}<br>
...
function renderLibrary() {
  ...
  page.innerHTML = `
    ...
    <div id="libBody">${has ? renderLibGrid(LIBRARY) : '<div class="lib-empty">Henüz kelime eklemedin.<br>Okuyucudan kitap yükleyip kelimelerin üzerine gelerek ekleyebilirsin.</div>'}</div>
  `;
  $('#libSearch').oninput = (e) => {
...
/* ============================================================
   13) PRATİK — her sorunun altında açıklama
   ============================================================ */
const PRACTICE = {
  mode: 'infinite',
  q: null,
...
function renderPractice(initialMode) {
  ...
  $('#page-practice').innerHTML = `
    <div class="page-head">
      <h1>Pratik</h1>
      <p>Sonsuz sorularla kuralları pekiştir. <b>Her sorunun altında "Neden doğru/yanlış" açıklaması</b> çıkar.</p>
    </div>
    <div class="modes">
      <button class="mode-btn ${PRACTICE.mode === 'infinite' ? 'selected' : ''}" data-mode="infinite">
        <div class="mt">♾️ Sonsuz Pratik</div>
        <div class="md">Kurallı üretilen rastgele sorular + açıklama</div>
      </button>
...
function nextQuestion() {
  ...
  const q = PRACTICE.mode === 'flash' ? makeFlashQuestion() : makeInfiniteQuestion();
...
function makeInfiniteQuestion() {
  ...
  if (kind === 'case') {
    ...
    const g = noun.gender;
    const soft = /[яеёюьй]$/.test(noun.ru);
    const anim = noun.anim;

    let ruleTxt = '';
    if (c === 'gen') {
      if (g === 'm') ruleTxt = soft ? 'Eril yumuşak → **-я**' : 'Eril sert → **-а**';
      else if (g === 'f') ruleTxt = noun.ru.endsWith('а')
        ? (/[кгхжчшщ]а$/.test(noun.ru) ? 'Dişil -а (к/г/х/ж/ч/ш/щ sonrası) → **-и**' : 'Dişil -а → **-ы**')
        : 'Dişil -я/-ь → **-и**';
      else ruleTxt = soft ? 'Nötr yumuşak → **-я**' : 'Nötr sert → **-а**';
    } else if (c === 'dat') {
      if (g === 'm') ruleTxt = soft ? 'Eril yumuşak → **-ю**' : 'Eril sert → **-у**';
      else if (g === 'f') ruleTxt = 'Dişil → **-е** (veya -ь ise -и)';
      else ruleTxt = soft ? 'Nötr yumuşak → **-ю**' : 'Nötr sert → **-у**';
    } else if (c === 'acc') {
      if (g === 'm') ruleTxt = anim
        ? 'Eril **canlı** → Genitive gibi (-а/-я)'
        : 'Eril **cansız** → Nominative ile aynı (değişmez)';
      else if (g === 'f') ruleTxt = soft ? 'Dişil → **-ю**' : 'Dişil -а → **-у**';
      else ruleTxt = 'Nötr → Nominative ile aynı (değişmez)';
    } else if (c === 'ins') {
      if (g === 'm') ruleTxt = soft ? 'Eril yumuşak → **-ем**' : 'Eril sert → **-ом**';
      else if (g === 'f') ruleTxt = noun.ru.endsWith('ь') ? 'Dişil -ь → **-ью**'
        : (/[жчшщ]а$/.test(noun.ru) ? 'Dişil -а (ж/ч/ш/щ sonrası) → **-ей**' : 'Dişil -а/-я → **-ой/-ей**');
      else ruleTxt = soft ? 'Nötr yumuşak → **-ем**' : 'Nötr sert → **-ом**';
    } else if (c === 'pre') {
      ruleTxt = 'Tekil Prepositional → hemen hemen her cinsiyette **-е**';
    }
    ...
    return {
      ...
      explain: {
        why: `"<b>${noun.ru}</b>" ${g === 'm' ? 'eril' : g === 'f' ? 'dişil' : 'nötr'} bir isimdir. ${CASE_TR[c]} hâlinde sonu <b class="ru">${answer}</b> olur.`,
        rule: ruleTxt,
        example: `Tam çekim → Nom: <span class="ru">${noun.ru}</span> · Gen: <span class="ru">${forms.sg.gen}</span> · Dat: <span class="ru">${forms.sg.dat}</span> · Acc: <span class="ru">${forms.sg.acc}</span> · Ins: <span class="ru">${forms.sg.ins}</span> · Pre: <span class="ru">${forms.sg.pre}</span>`
      }
    };
  }
  ...
  if (kind === 'verb-pres') {
    ...
    const trPron   = ['ben','sen','o','biz','siz','onlar'];
    const answer = pr[i];
    if (!answer) return makeInfiniteQuestion();

    const conj = inf.endsWith('ить') ? 2 : 1;
    const ruleTxt = conj === 1
      ? 'I. çekim: я -ю · ты -ешь · он -ет · мы -ем · вы -ете · они -ют'
      : 'II. çekim: я -ю · ты -ишь · он -ит · мы -им · вы -ите · они -ят';

    return {
      type: 'text',
      label: `Fiili çekimle — ${trPron[i]} (${pronouns[i]})`,
      target: inf,
      answer,
      checkFn: v => norm(v) === norm(answer),
      explain: {
        why: `"<b>${inf}</b>" (${conj}. çekim) fiilinin <b>${pronouns[i]}</b> için çekimi <b class="ru">${answer}</b>'dir.`,
        rule: ruleTxt,
        example: `Tam çekim → я: <span class="ru">${pr[0]}</span> · ты: <span class="ru">${pr[1]}</span> · он/она: <span class="ru">${pr[2]}</span> · мы: <span class="ru">${pr[3]}</span> · вы: <span class="ru">${pr[4]}</span> · они: <span class="ru">${pr[5]}</span>`
      }
    };
  }
  ...
  if (kind === 'verb-past') {
    ...
    return {
      ...
      target: inf,
      answer: ans,
      checkFn: v => norm(v) === norm(ans),
      explain: {
        why: `Geçmiş zaman, öznenin cinsiyetine/sayısına göre değişir. <b>${lbl}</b> için doğru form <b class="ru">${ans}</b>.`,
        rule: 'Eril → -л · Dişil → -ла · Nötr → -ло · Çoğul → -ли',
        example: `Tam çekim → <span class="ru">${pa.m}</span> · <span class="ru">${pa.f}</span> · <span class="ru">${pa.n}</span> · <span class="ru">${pa.pl}</span>`
      }
    };
  }
  ...
  if (kind === 'adj-agree') {
    ...
    const g = noun.gender;
    const correctAdj = forms.nom[g];
    if (!correctAdj) return makeInfiniteQuestion();
    const answer = `${correctAdj} ${noun.ru}`;

    const gLabel = g === 'm' ? 'eril' : g === 'f' ? 'dişil' : 'nötr';
    const gRule = g === 'm' ? '-ый / -ий / -ой' : g === 'f' ? '-ая / -яя' : '-ое / -ее';
    ...
    return {
      ...
      checkFn: v => norm(v) === norm(answer),
      explain: {
        why: `"<b>${noun.ru}</b>" ismi <b>${gLabel}</b>'dir. Sıfat bu cinsiyete uyum sağlar: "<span class="ru">${adj.ru}</span>" → <b class="ru">${correctAdj}</b>.`,
        rule: `Sıfat-Nominative ekleri → Eril: -ый/-ий/-ой · Dişil: -ая/-яя · Nötr: -ое/-ее · Çoğul: -ые/-ие. Burada: <b>${gRule}</b>`,
        example: `Doğru tamlama: <b class="ru">${answer}</b>`
      }
    };
  }
  ...
  if (kind === 'translate-ru') {
    ...
    const posTr = { noun:'isim', verb:'fiil', adj:'sıfat', adv:'zarf', int:'ünlem' }[e.pos] || e.pos;
    return {
      type: 'choice',
      ...
      answer: e.tr,
      explain: {
        why: `<b class="ru">${e.ru}</b> = <b>${e.tr}</b>`,
        rule: `Kelime türü: <b>${posTr}</b>${e.gender ? ' · cinsiyet: ' + (e.gender === 'm' ? 'eril' : e.gender === 'f' ? 'dişil' : 'nötr') : ''}`,
        example: e.pos === 'verb' ? `Mastar hâli "<span class="ru">${e.ru}</span>" — çekimlenince "-mek/-mak" anlamı taşır.` : ''
      }
    };
  }
  ...
  if (kind === 'translate-tr') {
    ...
    return {
      ...
      answer: e.ru,
      explain: {
        why: `<b>${e.tr}</b> = <b class="ru">${e.ru}</b>`,
        rule: `Kelime türü: <b>${posTr}</b>${e.gender ? ' · cinsiyet: ' + (e.gender === 'm' ? 'eril' : e.gender === 'f' ? 'dişil' : 'nötr') : ''}`,
        example: ''
      }
    };
...
function makeFlashQuestion() {
  ...
  if (dir === 'ru2tr') {
    ...
    if (wrongs.length < 3) {
      return {
        type: 'text',
        label: 'Türkçesi nedir? (yaz)',
        target: e.lemma,
        answer: e.tr,
        checkFn: v => norm(v) === norm(e.tr),
        explain: {
          why: `<b class="ru">${e.lemma}</b> = <b>${e.tr}</b>`,
          rule: e.pos ? `Kelime türü: <b>${e.pos}</b>${e.case ? ' · hâl: ' + CASE_TR[e.case] : ''}` : '',
          example: ''
        }
      };
    }
    return {
      ...
      explain: {
        why: `<b class="ru">${e.lemma}</b> = <b>${e.tr}</b>`,
        rule: e.pos ? `Kelime türü: <b>${e.pos}</b>${e.case ? ' · hâl: ' + CASE_TR[e.case] : ''}` : '',
        example: e.context ? `Bu kelimeyi şu bağlamda görmüştün: "${e.context}…"` : ''
      }
    };
  } else {
    return {
      ...
      explain: {
        why: `<b>${e.tr}</b> = <b class="ru">${e.lemma}</b>`,
        rule: e.pos ? `Kelime türü: <b>${e.pos}</b>${e.case ? ' · hâl: ' + CASE_TR[e.case] : ''}` : '',
        example: e.context ? `Bağlam: "${e.context}…"` : ''
      }
    };
  }
}
...
function result(ok, userAns, correctAns) {
  ...
  let explainHTML = '';
  if (q && q.explain) {
    const { why, rule, example } = q.explain;
    // **bold** işaretlerini <b> etiketine çevir
    const fmt = s => (s || '').replace(/\*\*(.+?)\*\*/g, '<b>$1</b>');
    explainHTML = `
      <div class="explain-box">
        <div class="row why"><b>📘 Neden?</b> ${why || '—'}</div>
        ${rule ? `<div class="row rule"><b>📐 Kural:</b> ${fmt(rule)}</div>` : ''}
        ${example ? `<div class="row detail"><b>🔎 Detay:</b> ${example}</div>` : ''}
      </div>
    `;
  }
  ...
  if (ok) {
    ...
    fb.innerHTML = `
      <h4>✅ Doğru!</h4>
      <div class="fb-word">${correctAns}</div>
      ${explainHTML}
    ...
    fb.innerHTML = `
      ...
      <div class="fb-word">${correctAns}</div>
      <div class="user-answer">Senin: ${userAns}</div>
      ${explainHTML}
      <button class="next" id="qNext">Sonraki Soru →</button>
    `;
  }
