/* ============================================================
   ...
   Sürüm: v6 (yatay popover, IndexedDB, ilerleme kaydı)
   ============================================================ */
...
function loadTransSettings() {
  try {
    TRANS_SETTINGS = { ...TRANS_SETTINGS, ...JSON.parse(localStorage.getItem(TRANS_SETTINGS_KEY) || '{}') };
  } catch (e) {}
}
...
/* ============================================================
   ...
   ============================================================ */
const CASE_TR = {
  nom:'Nominative (yalın)', gen:'Genitive (tamlayan)', dat:'Dative (yönelme)',
  acc:'Accusative (belirtme)', ins:'Instrumental (araç)', pre:'Prepositional (bulunma)'
};
...
/* ============================================================
   10) IndexedDB — Kitap Depolama + İlerleme
   ============================================================ */
...
async function renderHome() {
  ...
  $('#page-home').innerHTML = `
    ...
    <div class="features">
      <button class="feature" data-go="alphabet">
        <div class="icon">Аа</div><h3>Alfabe</h3><p>33 Kiril harfi, telaffuzu, örnekleri</p>
      </button>
      <button class="feature" data-go="grammar">
        <div class="icon">📐</div><h3>Gramer</h3><p>Hâller, cinsiyet, fiil çekimi, sıfat uyumu</p>
      </button>
      <button class="feature" data-go="reader">
        <div class="icon">📖</div><h3>Okuyucu</h3><p>EPUB/PDF yükle, kalıcı sakla, kelimeye tıkla</p>
      </button>
      <button class="feature" data-go="library">
        <div class="icon">📚</div><h3>Kütüphanem</h3><p>${bookCount} kitap · ${LIBRARY.length} kelime</p>
      </button>
      <button class="feature" data-go="practice">
        <div class="icon">🎯</div><h3>Pratik</h3><p>Sonsuz soru + flash kartlar</p>
      </button>
    </div>
    <h2>İlerlemen</h2>
    ...
    <div class="card" style="margin-top:20px">
      ...
      <ul style="margin:8px 0 0 20px; padding:0; font-size:14px; line-height:1.9">
        ...
        <li>Kelimenin üzerine gel → küçük pencere açılır</li>
        <li><b>📖</b> tuşu ile cümleyi çevir · <b>📐</b> çekim tablosu · <b>📚</b> diğer anlamlar</li>
        <li><b>+</b> tuşu ile kelimeyi kütüphanene kaydet</li>
      </ul>
    </div>
  `;
...
function renderAlphabet() {
  $('#page-alphabet').innerHTML = `
    <div class="page-head">
      ...
      <p>33 harf. Harfe tıkla → telaffuzu dinle.</p>
    </div>
...
function renderReader() {
  $('#page-reader').innerHTML = `
    <div class="page-head">
      ...
      <p>Kitap yükle → otomatik kaydedilir. Kaldığın yerden devam etmek için Kütüphanem'den aç.</p>
    </div>
...
function scheduleHide() {
  ...
  _hideTimer = setTimeout(() => {
    ...
    if (popState.targetEl && popState.targetEl.matches(':hover')) return;
    hidePopover();
  }, 200);
}
...
async function showPopover(wEl, pinned = false) {
...
async function fetchTranslationAsync(word) {
  ...
  if (trEl) trEl.innerHTML = '<span class="p-loading">⏳ Çeviri…</span>';
  ...
  if (tr) {
    ...
    if (el) el.innerHTML = '<span class="p-err">Çeviri alınamadı</span>';
  }
}
...
async function fetchSentenceTranslation() {
  ...
  if (el && el.isConnected) {
    el.innerHTML = tr ? '→ ' + tr : '<span style="color:#f87171">Çeviri alınamadı</span>';
    positionPopover(popState.targetEl);
  }
}
...
function renderPopover() {
  ...
  const caseTxt   = a.case && a.case !== '—' ? CASE_SHORT[a.case] : '';
  ...
  const hasTr     = !!a.tr;
  const hasLemma  = a.lemma && norm(a.lemma) !== norm(a.word);
  ...
  if (a.allForms && a.pos === 'noun') {
    ...
    formsHTML = `
      <b>Şimdiki</b>
      <div style="font-size:11.5px"><span class="ru">${a.allForms.pr.join(', ')}</span></div>
      <b style="margin-top:6px">Geçmiş</b>
      <div style="font-size:11.5px"><span class="ru">${a.allForms.pa.m}, ${a.allForms.pa.f}, ${a.allForms.pa.n}, ${a.allForms.pa.pl}</span></div>`;
  } else if (a.allForms && a.pos === 'adj') {
    const n = a.allForms.nom;
    formsHTML = `
      <b>Nominative</b>
      <ul>
        <li>eril: <span class="ru">${n.m}</span></li>
        <li>dişil: <span class="ru">${n.f}</span></li>
        <li>nötr: <span class="ru">${n.n}</span></li>
        <li>çoğul: <span class="ru">${n.p}</span></li>
      </ul>`;
  }
    ...
    meaningsHTML = `
      <ul>
        ${popState.meanings.slice(0, 6).map(m => `
          <li><span style="color:#9ca3af;font-size:10.5px">[${m.pos}]</span> ${m.text}</li>
        `).join('')}
      </ul>`;
  }

  /* --- Cümle --- */
  const sentenceHTML = popState.sentence ? `
  ...
  popover.innerHTML = `
    <div class="p-row">
      <div class="p-info">
        <div class="p-line1">
          <span class="p-word">${a.word}</span>
          ${hasLemma ? `<span class="p-lemma-inline">← <span class="ru">${a.lemma}</span></span>` : ''}
          ${a.pos && a.pos !== '?' ? `<span class="tag pos">${a.pos === 'noun' ? 'isim' : a.pos === 'verb' ? 'fiil' : a.pos === 'adj' ? 'sıfat' : a.pos}</span>` : ''}
          ${caseTxt ? `<span class="tag">${caseTxt}</span>` : ''}
          ${numTxt ? `<span class="tag">${numTxt}</span>` : ''}
          ${genderTxt ? `<span class="tag">${genderTxt}</span>` : ''}
          ${confTag}
        </div>
        <div class="p-tr">${hasTr ? a.tr : '<span class="p-loading">⏳ Çeviri…</span>'}</div>
      </div>
      <div class="p-buttons">
        <button class="p-btn" data-act="speak" title="Dinle">🔊</button>
        <button class="p-btn" data-panel="sentence" title="Cümleyi çevir" ${!popState.sentence ? 'style="display:none"' : ''}>📖</button>
        <button class="p-btn" data-panel="forms" title="Çekim tablosu" ${!formsHTML ? 'style="display:none"' : ''}>📐</button>
        <button class="p-btn" data-panel="meanings" title="Diğer anlamlar" ${!meaningsHTML ? 'style="display:none"' : ''}>📚</button>
        <button class="p-btn ${inLib ? 'added' : ''}" data-act="add" title="${inLib ? 'Eklendi' : 'Kütüphaneye ekle'}" ${inLib ? 'disabled' : ''}>${inLib ? '✓' : '+'}</button>
      </div>
    </div>

    <div class="p-panels ${(sentenceHTML || formsHTML || meaningsHTML) ? 'has-content' : ''}">
      <div class="p-panel" id="panel-sentence">${sentenceHTML}</div>
      <div class="p-panel" id="panel-forms">${formsHTML}</div>
      <div class="p-panel" id="panel-meanings">${meaningsHTML}</div>
    </div>
  `;

  /* --- Olaylar --- */
  popover.querySelector('[data-act="speak"]')?.addEventListener('click', e => {
    e.stopPropagation();
    speak(a.word);
  });
  ...
  popover.querySelectorAll('.p-btn[data-panel]').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
      const key = btn.dataset.panel;
      const panel = popover.querySelector('#panel-' + key);
      if (!panel) return;

      const willOpen = !panel.classList.contains('open');

      popover.querySelectorAll('.p-panel').forEach(p => p.classList.remove('open'));
      popover.querySelectorAll('.p-btn[data-panel]').forEach(b => b.classList.remove('active'));

      if (willOpen) {
        panel.classList.add('open');
        btn.classList.add('active');

        if (key === 'sentence' && !panel.dataset.translated) {
          panel.dataset.translated = '1';
  ...
  if (!LIBRARY.length) {
    body.innerHTML = `
      <div class="lib-empty">
        ...
        <b>Okuyucu</b>'dan bir kitap açıp kelimelerin üzerine gel → <b>+</b> ile kaydet.
      </div>`;
    return;
  }
...
function makeInfiniteQuestion() {
  ...
  if (kind === 'adj-agree') {
    ...
    return {
      type: 'text',
      label: `"${adj.tr}" + "${noun.ru}" — Nominative uyum`,
